...
## 2026-09-25 — Published-version update check

- Scope: answer whether the panel can ask GitHub for the current version, and implement it. No milestone is marked complete by this change.
- `src/entry.ts`: added `publishedVersion`, which reads `version` from `https://raw.githubusercontent.com/ChetanGk123/api-workbench/main/package.json` using the pristine `fetch` captured at launch (`directFetch`), so the check cannot be mocked, delayed or recorded by the workbench's own rules. A 5s `AbortSignal.timeout`, a non-OK response, a parse failure or a version that is not dotted-numeric all resolve to `undefined`. `checkForUpdate` now reports a newer published version or confirms the published version matches, and otherwise falls through to the existing origin-recorded comparison unchanged.
- `scripts/build.mjs`: the no-external-URL assertion now strips one allow-listed manifest URL, and asserts it is present, so the guard still rejects any other absolute URL, `import(`, `eval(` or `new Function`. The bundle loads no remote code; this URL is read as data.
- `src/ui/screens.ts`: the About card's hint and progress text no longer claim nothing is requested from the network, and the Settings summary point names the check.
- `tests/settings.spec.mjs`: tests now route the manifest URL rather than depending on the real repository or on being online. Added coverage for a newer published version (`99.9.9`), for the published version equalling this build, and for an aborted read falling back to the origin note.
- `README.md`, `docs/DEPLOYMENT.md`: documented what the check reads, and the three conditions it depends on.
- Commands: `npm run build` passed (278,322 minified bytes; 401,578 encoded bookmark characters). `npx playwright test` passed 240/240 in 2.0m. Node v24.21.0.
- Measured, with `curl -H 'Origin: https://example.com'`: raw.githubusercontent and api.github.com both answer `Access-Control-Allow-Origin: *`; raw caches for 300s; unauthenticated api.github.com allows 60 requests/hour per IP. `api.github.com/repos/ChetanGk123/api-workbench` and `chetangk123.github.io/api-workbench/bookmarklet.txt` both returned 404, confirming the repository and its Pages site are not publicly readable today.
- Limitations: while the repository stays private every check falls back to the origin note — the new path is verified only against routed responses in Playwright, never against the live URL. A host page's `connect-src` can block the request and this is not detectable in advance. `main` may be ahead of what Pages has deployed. The bookmark itself cannot be replaced by script; the check only reports. No release tags exist, so `releases/latest` was not used. Not run: Edge, manual saved-bookmark installation, any live hosted verification.
- Decision: the bundle may make exactly one network request, for a version manifest read as data. The build guard enforces that this stays a single allow-listed URL.
- Repository made public by the user the same day. Measured afterwards: `api.github.com/repos/ChetanGk123/api-workbench` returns 200, and the manifest returns 200 with `Access-Control-Allow-Origin: *` and `cache-control: max-age=300`.
- Live round trip, run once from a throwaway Playwright spec against the fixture page with no routing: exactly one request to the manifest URL was observed and the About card reported `Up to date: 1.0.2 is the published version.` The spec was deleted rather than kept, so the suite stays offline-deterministic.
- Added a permanent test instead: under the fixture's `/policy/strict` page (`connect-src 'self'`), the check falls back to the origin note rather than throwing. 14/14 in `tests/settings.spec.mjs`.
- Pages: `POST repos/ChetanGk123/api-workbench/pages` answered 409 “already enabled”; `GET .../pages` shows `build_type: workflow`, `public: true`, `status: null` (never built). The earlier 422 plan restriction is gone now the repository is public.
- Deployed: run 36109629015 on `f64b8b6` passed both jobs. `index.html`, `install.html`, `bookmarklet.txt` and `sizes.json` all return 200 from `https://chetangk123.github.io/api-workbench/`, and the deployed `bookmarklet.txt` is byte-identical to the local build (sha256 `3a0f0b4c…`, 401,578 characters). The earlier run on `b952a2a` failed only in its `deploy` job, before Pages was enabled; its `build` job passed.
- End-to-end, run once from a throwaway spec using the *deployed* bookmarklet text against the fixture page: one `200` response from the manifest URL, and the About card reported `Up to date: 1.0.2 is the published version.` The spec was deleted.
- Not verified live: the newer-version branch, because the published version equals this build. It is covered only by the routed test. A version bump followed by a check from an older saved bookmark would exercise it for real.
- Not run: Edge, and installation as a genuine saved bookmark dragged from the hosted installer. Every launch in these checks was script injection, which is not proof of saved-bookmark behavior.
- Next: bump the version and re-deploy to exercise the newer-version branch against a bookmark saved from the previous build, then install from the hosted installer by hand in Chrome and Edge.

## 2026-09-25 — Copy the published bookmarklet, and read the deployed site

- Scope: a button to update when a newer version exists. Page script cannot reach the bookmarks bar in Chrome or Edge, so no bookmarklet can replace the bookmark that launched it. What ships is the closest honest thing: the published bookmarklet on the clipboard, for the user to paste into the bookmark's URL field. Nothing fetched is executed.
- Source of truth moved from `raw.githubusercontent.com/.../main/package.json` to `https://chetangk123.github.io/api-workbench/`. Measured: Pages answers `Access-Control-Allow-Origin: *` with `cache-control: max-age=600` for `bookmarklet.txt`, which the earlier entry assumed it would not. `version.json` and `bookmarklet.txt` are uploaded by the same deployment, so the version reported and the text handed over cannot disagree — reading the repository could report a `main` version that is not installable yet.
- `scripts/build.mjs`: emits `dist/version.json` from `package.json`; the no-external-URL guard now allows the site base URL and asserts it is present. `.github/workflows/deploy.yml`: stages `version.json`.
- `src/entry.ts`: `readPublished` for both files; `publishedVersion` still resolves `undefined` on any failure; `copyUpdate` fetches `bookmarklet.txt`, requires it to start with `javascript:` and to contain the promised version, then writes it to the clipboard. A deployment caught between uploading the two files is therefore refused rather than copied. `checkForUpdate` now resolves `{ message, update? }`.
- `src/ui/screens.ts`, `src/ui/shell.ts`: **Copy new bookmarklet** appears in the About card only once a newer version is known, reports the paste steps, and reports its own failure without touching the running build.
- Version bumped to 1.0.4 before pushing, at the user's instruction.
- Commands: `npm run build` passed (279,511 minified bytes; 403,271 encoded characters). `npx playwright test` passed 242/242 in 2.0m, including three new settings tests: the copy path against a routed site, a refused mismatched bookmarklet leaving the clipboard untouched, and the existing CSP fallback. Node v24.21.0.
- Limitations: no automatic update exists and none is claimed — the user still edits the bookmark by hand. The copy path is verified only against routed responses; clipboard permission was granted by the test context, and a real page may prompt or refuse. Not run: Edge, and a real saved-bookmark paste-and-relaunch cycle.
- Deployed: run on `7bf633a` passed both jobs. `version.json` serves `{"version":"1.0.4"}` and the deployed `bookmarklet.txt` is byte-identical to the local build (sha256 `fafa682d…`, 403,271 characters).
...
## 2026-09-25 — Landing page favicon

- Interpreted the requested “dav icon” as favicon. Added an embedded SVG favicon using the existing code logo and zinc colors in `scripts/showcase.mjs`; regenerated ignored `dist/index.html`. Updated `scripts/build.mjs` to exempt this specific data-URI icon link from the external-asset guard. No external asset or dependency added.
- Verification: `npm run build` initially failed because the guard rejected every link element; passed after the narrow data-URI exemption. A one-off `node --input-type=module -e` assertion verified exactly one SVG favicon, successful URI decoding, its SVG namespace, 32×32 viewBox, and reused code-logo path. `git diff --check` passed. Node v24.21.0. Generated landing page: 451,514 bytes; runtime unchanged.
- Not run: browser tab visual check, full runtime suite, Edge, saved-bookmark installation, or deployment. This page-only change does not advance a milestone; next remains outstanding Chrome/Edge release verification.
