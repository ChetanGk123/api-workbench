...
export function xhrAdapter(Captured: typeof XMLHttpRequest, pipeline: Pipeline): typeof XMLHttpRequest {
  return new Proxy(Captured, {
    construct(Target, args, newTarget) {
      ...
      for (const key of ['readyState', 'status', 'statusText', 'responseURL', 'response', 'responseText', 'responseXML']) {
        Object.defineProperty(xhr, key, { configurable: true, get() {
          if (!synthetic) return nativeGet(key);
          switch (key) {
            case 'readyState': return synthetic.state;
            case 'status': return synthetic.state >= 2 && !synthetic.failed ? 200 : 0;
            case 'statusText': return synthetic.state >= 2 && !synthetic.failed ? 'OK' : '';
            case 'responseURL': return synthetic.state >= 2 && !synthetic.failed ? url : '';
            case 'responseText':
      ...
      while (!Object.hasOwn(timeoutPrototype, 'timeout')) timeoutPrototype = Object.getPrototypeOf(timeoutPrototype);
      const timeoutDescriptor = Object.getOwnPropertyDescriptor(timeoutPrototype, 'timeout')!;
      Object.defineProperty(xhr, 'timeout', { configurable: true,
        get: () => timeoutDescriptor.get!.call(xhr),
        set: value => { timeoutDescriptor.set!.call(xhr, value); rescheduleTimeout(); },
      });
      xhr.open = function (verb: string, target: string | URL, asynchronous: boolean = true, username?: string | null, password?: string | null) {
      ...
      xhr.getResponseHeader = name => !synthetic ? getHeader(name) :
        synthetic.state >= 2 && !synthetic.failed && name.toLowerCase() === 'content-type' ? 'application/json' : null;
      xhr.getAllResponseHeaders = () => !synthetic ? getHeaders() :
        synthetic.state >= 2 && !synthetic.failed ? 'content-type: application/json\r\n' : '';
      xhr.abort = () => {
        if (!synthetic) { abort(); return; }
        if (sent) fail('abort');
      ...
      xhr.send = body => {
        ...
        const emit = (type: string) => {
          xhr.dispatchEvent(type === 'readystatechange' ? new Event(type) :
            new ProgressEvent(type, { lengthComputable: !state.failed && state.state >= 3,
              loaded: state.failed || state.state < 3 ? 0 : new TextEncoder().encode(mockBody).length,
              total: state.failed || state.state < 3 ? 0 : new TextEncoder().encode(mockBody).length }));
          return valid();
        };
        const clear = () => {
          clearTimeout(timer); clearTimeout(deadline); release();
          cancel = undefined; rescheduleTimeout = () => {};
        };
        cancel = clear;
        fail = kind => {
        ...
        const finish = () => {
          clear();
          state.state = 2; if (!emit('readystatechange') || !sent) return;
          state.state = 3; if (!emit('readystatechange') || !sent || !emit('progress') || !sent) return;
          const bytes = new TextEncoder().encode(mockBody);
          state.response = xhr.responseType === 'json' ? JSON.parse(mockBody) :
        ...
        rescheduleTimeout = () => {
          ...
          if (xhr.timeout > 0) deadline = setTimeout(() => fail('timeout'), Math.max(0, xhr.timeout - (performance.now() - started)));
        };
